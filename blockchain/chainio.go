...
func (b *BlockChain) FetchSpendJournal(targetBlock *wfcutil.Block) ([]SpentTxOut, error) {
	b.chainLock.RLock()
...
func dbFetchSpendJournalEntry(dbTx database.Tx, block *wfcutil.Block) ([]SpentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := wfcutil.NewBlock(b.chainParams.GenesisBlock)
...
func (b *BlockChain) initChainState() error {
	...
	err = b.db.View(func(dbTx database.Tx) error {
		...
		for ok := cursor.First(); ok; ok = cursor.Next() {
			...
			if lastNode == nil {
				...
				if !blockHash.IsEqual(b.chainParams.GenesisHash) {
					return AssertError(fmt.Sprintf("initChainState: Expected %s " +
						"first entry in block index to be genesis block, "+
						"found %s", b.chainParams.GenesisHash, blockHash))
				}
		...
		blockWeight := uint64(GetBlockWeight(wfcutil.NewBlock(&block)))
...
// raw block for the provided node, deserialize it, and return a wfcutil.Block
// with the height set.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*wfcutil.Block, error) {
	...
	block, err := wfcutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
...
func dbStoreBlock(dbTx database.Tx, block *wfcutil.Block) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(blockHeight int32) (*wfcutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*wfcutil.Block, error) {
