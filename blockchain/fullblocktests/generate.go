...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: wfcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := wfcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*wfcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, wfcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount wfcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee wfcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee wfcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee wfcutil.Amount) *wire.MsgTx {
