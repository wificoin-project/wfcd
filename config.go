...
const (
	defaultConfigFilename        = "wfcd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "wfcd.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = wfcutil.AppDataDir("wfcd", false)
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in WFC/kB to be considered a non-zero fee."`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		wfcdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = wfcutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]wfcutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := wfcutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		wfcdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-wfcd.conf to the given destination path,
...
// wfcdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func wfcdDial(addr net.Addr) (net.Conn, error) {
...
// wfcdLookup resolves the IP of the given host using the correct DNS lookup
...
func wfcdLookup(host string) ([]net.IP, error) {
