...
wfcd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original wificoind/wificoin-qt.  There are a few key
differences between wfcd and wificoind as far as how RPCs are serviced:
* Unlike wificoind that has the wallet and chain intermingled in the same process
  which leads to several issues, wfcd intentionally splits the wallet and chain
  services into independent processes.  See the blog post
  [here](https://blog.conformal.com/wfcd-not-your-moms-wificoin-daemon/) for
  further details on why they were separated.  This means that if you are
  talking directly to wfcd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [btcwallet](https://github.com/btcsuite/btcwallet).
* wfcd is secure by default which means that the RPC connection is TLS-enabled
  by default
* wfcd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for wfcd RPC and are used by applications
such as [btcwallet](https://github.com/btcsuite/btcwallet) for inter-process
communication with wfcd.  The websocket connection endpoint for wfcd is
...
The original wificoind/wificoin-qt JSON-RPC API documentation is available at [https://en.wificoin.it/wiki/Original_Bitcoin_client/API_Calls_list](https://en.wificoin.it/wiki/Original_Bitcoin_client/API_Calls_list)

<a name="HttpPostVsWebsockets" />

### 2. HTTP POST Versus Websockets

The wfcd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
wfcd is `wss://your_ip_or_domain:8334/ws`.
...
to a wfcd RPC server:

* **rpcuser** is the full-access username configured for the wfcd RPC server
* **rpcpass** is the full-access password configured for the wfcd RPC server
* **rpclimituser** is the limited username configured for the wfcd RPC server
* **rpclimitpass** is the limited password configured for the wfcd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the wfcd
  server is configured with.  It is automatically generated by wfcd and placed
  in the wfcd home directory (which is typically `%LOCALAPPDATA%\Btcd` on
  Windows and `~/.wfcd` on POSIX-like OSes)

**NOTE:** As mentioned above, wfcd is secure by default which means the RPC
...
The wfcd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
wfcd comes with a separate utility named `btcctl` which can be used to issue
these RPC commands via HTTP POST requests to wfcd after configuring it with the
...
API compatible with the original wificoind/wificoin-qt client.
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">wfcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since wfcd does not have the wallet integrated to provide payment addresses, wfcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown wfcd.|
|28|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|29|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since wfcd does not have a wallet integrated, wfcd will only return whether the address is valid or not.|
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a wfcd extension**</font>|
...
|Notes|NOTE: Since wfcd does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in btcwallet for a version which includes that information.|
...
|Notes|<font color="orange">Since wfcd does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
|Returns (verbose=false)|`[ (json array of string)`<br />&nbsp;&nbsp;`"transactionhash", (string) hash of the transaction`<br />&nbsp;&nbsp;`...`<br />`]`|
|Returns (verbose=true)|`{ (json object)`<br />&nbsp;&nbsp;`"transactionhash": { (json object)`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"size": n, (numeric) transaction size in bytes`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"vsize": n, (numeric) transaction virtual size`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"fee" : n, (numeric) transaction fee in wificoins`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"time": n, (numeric) local time transaction entered pool in seconds since 1 Jan 1970 GMT`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"height": n, (numeric) block height when transaction entered the pool`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"startingpriority": n, (numeric) priority when transaction entered the pool`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"currentpriority": n, (numeric) current priority`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"depends": [ (json array) unconfirmed transactions used as inputs for this transaction`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"transactionhash", (string) hash of the parent transaction`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`...`<br />&nbsp;&nbsp;&nbsp;&nbsp;`]`<br />&nbsp;&nbsp;`}, ...`<br />`}`|
...
|Notes|NOTE: Since wfcd does not have the wallet integrated to provide payment addresses, wfcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">wfcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown wfcd.|
|Returns|`"wfcd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. address (string, required) - wificoin address|
|Description|Verify an address is valid.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"isvalid": true or false,  (bool) whether or not the address is valid.`<br />&nbsp;&nbsp;`"address": "wificoinaddress", (string) the wificoin address validated.`<br />}|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For wfcd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Btcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for wfcd.</font>|
...
The following is an overview of the RPC methods which are implemented by wfcd, but not the original wificoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get wificoin network wfcd is running on.|None|
...
|Parameters|None|
|Description|Get wificoin network wfcd is running on.|
|Returns|numeric|
...
|Parameters|1. Addresses (JSON array, required)<br />&nbsp;`[ (json array of strings)`<br />&nbsp;&nbsp;`"wificoinaddress", (string) the wificoin address`<br />&nbsp;&nbsp;`...`<br />&nbsp;`]`|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this wfcd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
|Parameters|1. BeginBlock (string, required) block hash to begin rescanning from<br />2. Addresses (JSON array, required)<br />&nbsp;`[ (json array of strings)`<br />&nbsp;&nbsp;`"wificoinaddress", (string) the wificoin address`<br />&nbsp;&nbsp;`...` <br />&nbsp;`]`<br />3. Outpoints (JSON array, required)<br />&nbsp;`[ (JSON array)`<br />&nbsp;&nbsp;`{ (JSON object)`<br />&nbsp;&nbsp;&nbsp;`"hash":"data", (string) the hex-encoded bytes of the outpoint hash`<br />&nbsp;&nbsp;&nbsp;`"index":n (numeric) the txout index of the outpoint`<br />&nbsp;&nbsp;`},`<br />&nbsp;&nbsp;`...`<br />&nbsp;`]`<br />4. EndBlock (string, optional) hash of final block to rescan|
...
wfcd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll wfcd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a wfcd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by wfcd when it starts the RPC server and doesn't already
	// have one.
	wfcdHomeDir := btcutil.AppDataDir("wfcd", false)
	certs, err := ioutil.ReadFile(filepath.Join(wfcdHomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[btcrpcclient](https://github.com/btcsuite/btcrpcclient) package to connect with
a wfcd instance via Websockets, issues [getblock](#getblock) to retrieve
...
import (
	"github.com/btcsuite/btcrpcclient"
	"github.com/btcsuite/btcutil"
	"github.com/btcsuite/wfcd/chaincfg/chainhash"
	"github.com/btcsuite/wfcd/wire"
	"io/ioutil"
	"log"
	"path/filepath"
	"time"
...
The following is an example Go application which uses the
[btcrpcclient](https://github.com/btcsuite/btcrpcclient) package to connect with
a wfcd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a wfcd instance,
...
// generated by wfcd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/wfcd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The wfcd generated certificate acts as
