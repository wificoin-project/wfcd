...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt wfcutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(wfcutil.Amount(5 * wfcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(wfcutil.Amount(500 * wfcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(wfcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*wfcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, wfcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := wfcutil.Amount(250 * wfcutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = wfcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := wfcutil.Amount(50 * wfcutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := wfcutil.Amount(numMatureOutputs * 50 * wfcutil.SatoshiPerBitcoin)
