...
// the given fee rate.
func (rate SatoshiPerByte) Fee(size uint32) wfcutil.Amount {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1) {
		return wfcutil.Amount(-1)
	}

	return wfcutil.Amount(float64(rate) * float64(size))
}

// NewSatoshiPerByte creates a SatoshiPerByte from an Amount and a
// size in bytes.
func NewSatoshiPerByte(fee wfcutil.Amount, size uint32) SatoshiPerByte {
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewSatoshiPerByte(wfcutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *wfcutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*wfcutil.Tx]struct{})
