...
const (
	...
	DefaultMinRelayTxFee = wfcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee wfcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > wfcutil.MaxSatoshi {
		minFee = wfcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *wfcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee wfcutil.Amount) bool {
...
func checkTransactionStandard(tx *wfcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee wfcutil.Amount,
...
func GetTxVirtualSize(tx *wfcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
