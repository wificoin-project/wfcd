...
// newAddressPubKey returns a new wfcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) wfcutil.Address {
	addr, err := wfcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new wfcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) wfcutil.Address {
	addr, err := wfcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new wfcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) wfcutil.Address {
	addr, err := wfcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the wfcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the wfcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := wfcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := wfcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := wfcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := wfcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := wfcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*wfcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*wfcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*wfcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*wfcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*wfcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
